      ...
      <section className="border-b border-neutral-900">
        <div className="mx-auto max-w-7xl px-6 py-28">
          <div className="mb-16 flex items-end justify-between">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.35em] text-[#C9A66B]">Selected Works</p>

              <h2 className="mt-4 font-serif text-5xl text-white">Featured Projects</h2>
            </div>

            {projectPreview.length > 0 && (
              <Link to="/portfolio" className="text-sm text-neutral-400 transition hover:text-white">
                View All →
              </Link>
            )}
          </div>
              ...
              {projectPreview.map((project, index) => (
                ...
                >
                  <div className="grid items-center gap-10 lg:grid-cols-[100px_1fr_auto]">
                    ...
                    <div>
                      <h3 className="text-3xl font-semibold text-white">{project.proj_name}</h3>

                      <p className="mt-4 max-w-2xl text-neutral-400 line-clamp-2">
                        {project.project_details?.description ?? '프로젝트 설명이 등록되지 않았습니다.'}
                      </p>
                    ...
                    <div className="space-y-2 text-right">
                      <p className="text-xs uppercase tracking-[0.25em] text-neutral-600">Duration</p>

                      <p className="text-sm text-neutral-300">{formatDateKST(project.start_date)}</p>

                      <p className="text-sm text-neutral-500">~ {formatDateKST(project.end_date)}</p>
                    </div>
                  </div>
                </Link>
              ))}
